...
directories: (字符串数组) 要监控的目录路径列表。

functions: (字符串数组) 在这些目录的文件中需要监控的函数或方法名列表。
...
实现: 通过 CallVisitor 访问器遍历文件的 AST，找出所有函数调用表达式（ExprCall）。


过滤: 访问器能够智能识别并过滤掉位于测试代码块（#[test], #[tokio::test], #[cfg(test)]）中的调用，避免测试代码的干扰。


比较: 将新旧版本中所有非测试代码的函数调用点分别存入两个 HashSet。通过比较集合的差异，精确地判断指定函数的调用点是否被新增或删除。
